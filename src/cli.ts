...
const AGENTS = [
	{ name: "claude", cmd: (p: string) => ["claude", "--dangerously-skip-permissions", "--model", "sonnet", p] },
	{ name: "codex", cmd: (p: string) => ["codex", "--approval-mode", "full-auto", p] },
	{ name: "opencode", cmd: (p: string) => ["opencode", p] },
];
...
async function main() {
	...
	// detect agents
	const available = AGENTS.filter((a) => Bun.which(a.name));
